...
func GetEventsWithLimit(minID, maxID int) (events []models.Events, lastId int, err error) {
	fmt.Println("Obteniendo eventos con entre", minID, "y", maxID)
	...
	rows, err := db.Query(`
        ...
        WHERE
            ...
            AND e.sended = 0
			AND e.id > ? AND e.id <= ?
        LIMIT ?`, minID, maxID, 1000)
	if err != nil {
		return nil, minID, fmt.Errorf("error al preparar la consulta: %w", err)
	}
	defer rows.Close()
	...
	if err := rows.Err(); err != nil {
		return nil, minID, fmt.Errorf("error durante la iteración de resultados: %w", err)
	}
...
func GetEventsWithOutstartID(limit int) (events []models.Events, lastId int, err error) {
	// Inicializa el slice de eventos
	events = make([]models.Events, 0)
	...
	if err != nil {
		return nil, 0, fmt.Errorf("error al preparar la consulta: %w", err)
	}
	defer rows.Close()
	// Loop through rows, using Scan to assign column data to struct fields.
	for rows.Next() {
	...
	if err := rows.Err(); err != nil {
		return nil, 0, fmt.Errorf("error durante la iteración de resultados: %w", err)
	}
	fmt.Println("Función inicial extrajo los datos con el last id", events[len(events)-1].ID)
	return events, events[len(events)-1].ID, nil
}
